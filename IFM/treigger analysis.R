...
#leads3 <- leads2 %>% filter( !(year == 2014 & month %in% 7:10)) # to replicate iqr
...
#sales4 <- sales3 %>% filter( !(year == 2014 & month %in% 7:10))  #replicate theirs
...
analysis3 %>% group_by(Channel,Trigger) %>% summarise(leads=sum(leads_test), incr_contr_90=sum(incr_contr_90),incr_contr_95=sum(incr_contr_95))
...
analysis3 %>% group_by(Channel,Trigger,Product) %>% summarise_each(funs(sum),c(incr_contr_95:incr_contrib_iqr_3y))
...
analysis3 %>% group_by(Channel,Trigger) %>% summarise_each(funs(mean),c(leads_test))
...
analysis4 <- analysis3 %>% group_by(Channel,Trigger) %>% summarise_each(funs(sum),sales_control:leads_test) %>%
  mutate(lift=(sales_test/(leads_test/12))-(sales_control/(leads_control/12)))

analysis4$p_value=apply(analysis4[c("sales_test","sales_control","leads_test","leads_control")],1, function(x) mytest(x[1],x[2],x[3]/12,x[4]/12))
...
# analyze all by channel and product
analysis5 <- analysis3 %>% group_by(Channel,Product) %>% summarise_each(funs(sum),sales_control:leads_test) %>%
  mutate(lift=(sales_test/(leads_test/12))-(sales_control/(leads_control/12)))

analysis5$p_value=apply(analysis5[c("sales_test","sales_control","leads_test","leads_control")],1, function(x) mytest(x[1],x[2],x[3],x[4]))

analysis5$p_value1=apply(analysis5[c("sales_test","sales_control","leads_test","leads_control")],1, function(x) mytest(x[2],x[1],x[4],x[3]))

analysis5$incremental <- ifelse(analysis5$p_value <=0.1,floor(analysis5$leads_test*analysis5$lift),0)
analysis5$lift = with(analysis5,sales_test/leads_test - sales_control/leads_control)

analysis5$p1 <- format(analysis5$p_value,digits=5,scientific=F)
